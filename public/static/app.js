...
// ==================== CHECKOUT ====================
let _checkoutCoupon=null;
function showCheckout(){
  _checkoutCoupon=null;
  ...
  m.innerHTML=`<div class="modal-content checkout-modal">
    ...
    <div class="checkout-section checkout-summary-section">
      ...
      <div id="coSubtotalRow" class="checkout-total-row" style="font-weight:500;font-size:.9rem"><span>${t('subtotal')}</span><span>${formatPrice(total)}</span></div>
      <div id="coDiscountRow" style="display:none;padding:.4rem 0;color:var(--success);font-size:.85rem;display:none;justify-content:space-between"><span><i class="fas fa-tag" style="margin-inline-end:.3rem"></i>${t('discountApplied')}</span><span id="coDiscountAmt">0</span></div>
      <div class="checkout-total-row"><span>${t('total')}</span><span class="checkout-total-price" id="coTotalPrice">${formatPrice(total)}</span></div>
    </div>
    <div class="checkout-section"><div class="checkout-section-title"><i class="fas fa-ticket"></i><span>${t('couponCode')}</span></div>
      <div style="display:flex;gap:.5rem;align-items:start"><input type="text" id="coCoupon" class="form-input" placeholder="${t('enterCoupon')}" style="flex:1;text-transform:uppercase;font-family:monospace;letter-spacing:1px">
      <button type="button" class="btn-primary btn-sm" id="coCouponBtn" onclick="applyCouponCheckout()" style="white-space:nowrap;height:40px"><i class="fas fa-check"></i> ${t('applyCoupon')}</button></div>
      <div id="coCouponMsg" style="font-size:.78rem;margin-top:.4rem"></div>
    </div>
...
async function applyCouponCheckout(){
  ...
  const totalEl=document.getElementById('coTotalPrice');
  const btn=document.getElementById('coCouponBtn');
  if(!code){msgEl.innerHTML=`<span style="color:var(--error)">${t('enterCoupon')}</span>`;return}

  // If already applied, remove it
  if(_checkoutCoupon&&_checkoutCoupon.code===code.toUpperCase()){
    _checkoutCoupon=null;
    const cartTotal=Store.cart.reduce((s,i)=>s+i.price*i.quantity,0);
    discRow.style.display='none';
    totalEl.textContent=formatPrice(cartTotal);
    msgEl.innerHTML='';
    btn.innerHTML=`<i class="fas fa-check"></i> ${t('applyCoupon')}`;
    document.getElementById('coCoupon').value='';
    return}

  btn.innerHTML='<div class="loading-spinner" style="width:16px;height:16px"></div>';
  const cartTotal=Store.cart.reduce((s,i)=>s+i.price*i.quantity,0);
  const{ok,data}=await API.post('/api/coupons/validate',{code,order_total:cartTotal});
  if(ok&&data.valid){
    _checkoutCoupon={code:data.code,discount_amount:data.discount_amount,new_total:data.new_total};
    discRow.style.display='flex';
    discAmt.textContent=`- ${formatPrice(data.discount_amount)}${data.discount_type==='percentage'?' ('+data.discount_value+'%)':''}`;
    totalEl.textContent=formatPrice(data.new_total);
    msgEl.innerHTML=`<span style="color:var(--success)"><i class="fas fa-check-circle"></i> ${t('couponApplied')} - ${data.discount_type==='percentage'?data.discount_value+'%':formatPrice(data.discount_value)} ${t('discount')}</span>`;
    btn.innerHTML=`<i class="fas fa-times"></i> ${t('removeCoupon')}`;
  } else {
    _checkoutCoupon=null;
    discRow.style.display='none';totalEl.textContent=formatPrice(cartTotal);
    const errKey=data.error;
    const errMsg=errKey==='invalid_code'?t('invalidCoupon'):errKey==='coupon_expired'?t('couponExpired'):errKey==='coupon_maxed'?t('couponMaxed'):errKey==='coupon_inactive'?t('couponInactive'):errKey==='min_order_not_met'?(t('minOrderNotMet')+` (${formatPrice(data.min_order||0)})`):t('invalidCoupon');
    msgEl.innerHTML=`<span style="color:var(--error)"><i class="fas fa-times-circle"></i> ${errMsg}</span>`;
    btn.innerHTML=`<i class="fas fa-check"></i> ${t('applyCoupon')}`;
  }}
...
async function placeOrder(){
  ...
  const{ok,data}=await API.post('/api/orders',{full_name:fn,phone:ph,phone2:ph2,email:em,governorate:gov,city:city,address:addr,notes,coupon_code:_checkoutCoupon?.code||''});
...
<div class="total-section"><p>${t('subtotal')}: ${formatPrice(order.subtotal||order.total)}</p>${order.coupon_code?`<p style="color:green">${t('couponCode')}: ${order.coupon_code} — ${t('discount')}: -${formatPrice(order.discount_amount||0)}</p>`:`<p>${t('shipping')}: ${Store.lang==='ar'?'حسب المنطقة':'Based on location'}</p>`}<p class="grand">${t('grandTotal')}: ${formatPrice(order.total)}</p></div>
...
async function loadAdminPage(){
  ...
  const _lowStockProds=stats.lowStockProducts||[];const _outOfStockProds=stats.outOfStockProducts||[];
  window._dashLowStock=_lowStockProds;window._dashOutOfStock=_outOfStockProds;
    ...
    <div class="dash-grid">
      ...
      <div>
        ...
        <div class="dash-section animate-on-scroll" style="margin-bottom:1rem">
          ...
          <div style="display:flex;gap:.6rem"><div onclick="showStockModal('low')" style="flex:1;padding:.6rem;border-radius:var(--radius-sm);background:rgba(245,158,11,.08);text-align:center;cursor:pointer;transition:transform .15s,box-shadow .15s" onmouseover="this.style.transform='scale(1.04)';this.style.boxShadow='0 2px 12px rgba(245,158,11,.15)'" onmouseout="this.style.transform='';this.style.boxShadow=''"><div style="font-size:1.3rem;font-weight:800;color:#f59e0b">${stats.lowStock||0}</div><div style="font-size:.7rem;color:var(--text-secondary)">${t('lowStock')}</div></div><div onclick="showStockModal('out')" style="flex:1;padding:.6rem;border-radius:var(--radius-sm);background:rgba(239,68,68,.08);text-align:center;cursor:pointer;transition:transform .15s,box-shadow .15s" onmouseover="this.style.transform='scale(1.04)';this.style.boxShadow='0 2px 12px rgba(239,68,68,.15)'" onmouseout="this.style.transform='';this.style.boxShadow=''"><div style="font-size:1.3rem;font-weight:800;color:#ef4444">${stats.outOfStock||0}</div><div style="font-size:.7rem;color:var(--text-secondary)">${t('outOfStockCount')}</div></div></div>
        </div>
        <div class="dash-section animate-on-scroll">
...
function showStockModal(type){
  ...
  const products=isLow?(window._dashLowStock||[]):(window._dashOutOfStock||[]);
  const title=isLow?(Store.lang==='ar'?'منتجات مخزون منخفض':'Low Stock Products'):(Store.lang==='ar'?'منتجات نفدت':'Out of Stock Products');
  const icon=isLow?'fa-exclamation-triangle':'fa-times-circle';
  const color=isLow?'#f59e0b':'#ef4444';
  const m=document.createElement('div');m.className='modal-overlay';m.onclick=e=>{if(e.target===m)m.remove()};
  m.innerHTML=`<div class="modal-content" style="max-width:650px;max-height:85vh;overflow-y:auto">
    <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:1.2rem;padding-bottom:.8rem;border-bottom:2px solid ${color}20">
      <div style="width:40px;height:40px;border-radius:10px;background:${color}15;display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="fas ${icon}" style="color:${color};font-size:1.1rem"></i></div>
      <div><h2 style="font-size:1.1rem;margin:0">${title}</h2><p style="font-size:.75rem;color:var(--text-secondary);margin:0">${products.length} ${Store.lang==='ar'?'منتج':'product(s)'}</p></div>
      <button onclick="this.closest('.modal-overlay').remove()" style="margin-inline-start:auto;background:none;border:none;cursor:pointer;font-size:1.2rem;color:var(--text-secondary);padding:4px"><i class="fas fa-times"></i></button>
    </div>
    ${products.length?`<div class="admin-list" style="gap:.5rem">${products.map(p=>{
      const stockColor=p.stock===0?'#ef4444':p.stock<=2?'#f59e0b':'#fb923c';
      return`<div class="admin-row" style="cursor:pointer;padding:.7rem" onclick="this.closest('.modal-overlay').remove();Router.navigate('/admin/edit-product/${p.id}')">
        <img src="${p.product_img||'/static/images/1.png'}" onerror="this.src='/static/images/1.png'" style="width:44px;height:44px;border-radius:8px;object-fit:cover;flex-shrink:0">
        <div class="admin-row-info" style="min-width:0"><h4 style="font-size:.85rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${getTitle(p)}</h4><p style="font-size:.72rem">${p.category||''}</p></div>
        <div style="display:flex;align-items:center;gap:.5rem;flex-shrink:0">
          <span style="background:${stockColor}15;color:${stockColor};font-size:.75rem;font-weight:700;padding:.25rem .6rem;border-radius:20px;white-space:nowrap"><i class="fas fa-cubes" style="margin-inline-end:.25rem"></i>${p.stock} ${Store.lang==='ar'?'قطعة':'pcs'}</span>
          <span style="color:var(--accent);font-weight:600;font-size:.8rem;white-space:nowrap">${formatPrice(p.price)}</span>
        </div>
      </div>`}).join('')}</div>`
    :`<div class="empty-state" style="padding:2rem"><i class="fas ${isLow?'fa-check-circle':'fa-box-open'}" style="font-size:2.5rem;color:var(--success)"></i><h3 style="margin-top:.8rem">${Store.lang==='ar'?'لا توجد منتجات':'No products'}</h3></div>`}
  </div>`;document.body.appendChild(m)}
  ...
  renderPage(`<div class="admin-page-inner">
    ...
    <div class="order-detail-grid">
      ...
      <div class="animate-fadeInRight">
        <div class="admin-form" style="margin-bottom:1rem"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.8rem"><h3 style="font-size:1rem;display:flex;align-items:center;gap:.4rem;margin:0"><i class="fas fa-shopping-bag" style="color:var(--accent)"></i>${t('orderItems')}</h3>
        <button class="btn-primary btn-sm" onclick="showAddItemModal('${order.id}')"><i class="fas fa-plus"></i> ${t('addItemToOrder')}</button></div>
        ${(order.items||[]).map((it,idx)=>`<div style="display:flex;gap:.6rem;padding:.6rem 0;border-bottom:1px solid var(--border);align-items:center">
          ...
          <span style="font-weight:700;color:var(--accent);font-size:.9rem;margin-inline-end:.4rem">${formatPrice(it.price*it.quantity)}</span>
          <button class="icon-btn" style="width:28px;height:28px;font-size:.65rem;background:rgba(239,68,68,.1);color:var(--error);border-color:rgba(239,68,68,.2);flex-shrink:0" onclick="removeOrderItem('${order.id}',${idx})" title="${t('removeItemFromOrder')}"><i class="fas fa-trash"></i></button></div>`).join('')}
        <div style="display:flex;justify-content:space-between;margin-top:1rem;padding-top:.8rem;border-top:2px solid var(--accent);font-weight:800;font-size:1.1rem"><span>${t('total')}</span><span style="color:var(--accent)">${formatPrice(order.total)}</span></div>
        ${order.coupon_code?`<div style="margin-top:.5rem;padding:.5rem .8rem;background:rgba(34,197,94,.06);border-radius:var(--radius-sm);border:1px solid rgba(34,197,94,.15);display:flex;justify-content:space-between;align-items:center"><span style="font-size:.8rem;color:var(--success)"><i class="fas fa-ticket" style="margin-inline-end:.3rem"></i>${t('couponCode')}: <strong style="font-family:monospace">${order.coupon_code}</strong></span><span style="font-size:.8rem;color:var(--success);font-weight:700">-${formatPrice(order.discount_amount||0)}</span></div>`:''}</div>
...
async function removeOrderItem(orderId,itemIndex){
  ...
  const{ok}=await API.put(`/api/orders/${orderId}`,{action:'remove_item',item_index:itemIndex});
  if(ok){showToast(t('itemRemoved'));loadOrderDetailPage(orderId)}else showToast(t('error'),'error')}

async function showAddItemModal(orderId){
  const _ap=await API.get('/api/products');const products=Array.isArray(_ap)?_ap:[];
  const m=document.createElement('div');m.className='modal-overlay';m.id='addItemModal';
  m.onclick=e=>{if(e.target===m)m.remove()};
  m.innerHTML=`<div class="share-modal" style="max-width:550px;max-height:85vh;display:flex;flex-direction:column">
    <div class="share-modal-header"><h3><i class="fas fa-plus-circle" style="color:var(--accent)"></i> ${t('addItemToOrder')}</h3><button class="modal-close-btn" onclick="document.getElementById('addItemModal').remove()"><i class="fas fa-times"></i></button></div>
    <div style="margin-bottom:.8rem"><input type="text" class="form-input" id="addItemSearch" placeholder="${t('searchProducts')}" oninput="filterAddItemList()"></div>
    <div id="addItemList" style="overflow-y:auto;flex:1;max-height:50vh">
      ${products.map(p=>`<div class="add-item-row" data-title="${(p.title_ar||'').toLowerCase()} ${(p.title_en||'').toLowerCase()}" style="display:flex;gap:.6rem;padding:.6rem;border-bottom:1px solid var(--border);align-items:center;cursor:pointer;border-radius:var(--radius-sm);transition:var(--transition)" onmouseover="this.style.background='rgba(201,168,76,.05)'" onmouseout="this.style.background=''" onclick="doAddItemToOrder('${orderId}','${p.id}')">
        <img src="${p.product_img||'/static/images/1.png'}" style="width:40px;height:40px;border-radius:6px;object-fit:cover" onerror="this.src='/static/images/1.png'">
        <div style="flex:1;min-width:0"><p style="font-size:.82rem;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${getTitle(p)}</p><p style="font-size:.72rem;color:var(--text-secondary)">${p.category||''} · ${t('productStock')}: ${p.stock}</p></div>
        <span style="font-weight:700;color:var(--accent);font-size:.85rem;white-space:nowrap">${formatPrice(p.price)}</span>
      </div>`).join('')}
    </div>
  </div>`;
  document.body.appendChild(m)}
function filterAddItemList(){
  const q=(document.getElementById('addItemSearch')?.value||'').toLowerCase();
  document.querySelectorAll('.add-item-row').forEach(r=>{r.style.display=!q||r.dataset.title.includes(q)?'flex':'none'})}
async function doAddItemToOrder(orderId,productId){
  const{ok}=await API.put(`/api/orders/${orderId}`,{action:'add_item',product_id:productId,quantity:1});
  document.getElementById('addItemModal')?.remove();
  if(ok){showToast(t('itemAdded'));loadOrderDetailPage(orderId)}else showToast(t('error'),'error')}

// ==================== ADMIN COUPONS ====================
async function loadAdminCouponsPage(){
  if(!Store.isAdmin()){Router.navigate('/signin');return}
  const _ac=await API.get('/api/coupons');const coupons=Array.isArray(_ac)?_ac:[];
  const now=new Date();
  renderPage(`<div class="admin-page-inner">
    <div class="admin-page-header"><h2><i class="fas fa-ticket"></i> ${t('manageCoupons')}</h2>
    <button class="btn-primary btn-sm" onclick="showCouponForm()"><i class="fas fa-plus"></i> ${t('addCoupon')}</button></div>
    <div class="admin-list">${coupons.map(c=>{
      const expired=c.expires_at&&new Date(c.expires_at)<now;
      const maxed=c.max_uses>0&&c.used_count>=c.max_uses;
      const statusTxt=!c.is_active?t('inactive'):expired?t('couponExpired'):maxed?t('couponMaxed'):t('active');
      const statusCls=!c.is_active?'status-cancelled':expired?'status-cancelled':maxed?'status-shipped':'status-completed';
      const discTxt=c.discount_type==='percentage'?`${c.discount_value}%`:formatPrice(c.discount_value);
      return`<div class="admin-row">
        <div style="width:44px;height:44px;border-radius:10px;background:rgba(201,168,76,.1);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="fas fa-ticket" style="color:var(--accent)"></i></div>
        <div class="admin-row-info"><h4 style="font-family:monospace;letter-spacing:1px">${c.code}</h4>
        <p>${c.discount_type==='percentage'?t('percentage'):t('fixedAmount')}: <strong>${discTxt}</strong> · ${t('usedCount')}: ${c.used_count}/${c.max_uses||'∞'}${c.min_order>0?' · '+t('minOrder')+': '+formatPrice(c.min_order):''}${c.expires_at?' · '+new Date(c.expires_at).toLocaleDateString(Store.lang==='ar'?'ar-EG':'en-US'):''}</p></div>
        <span class="status-badge ${statusCls}">${statusTxt}</span>
        <div class="admin-row-actions">
          <button class="icon-btn" style="width:32px;height:32px;font-size:.75rem" onclick="showCouponForm(${JSON.stringify(c).replace(/"/g,'&quot;')})" title="${t('editCoupon')}"><i class="fas fa-edit"></i></button>
          <button class="icon-btn" style="width:32px;height:32px;font-size:.75rem;background:rgba(239,68,68,.1);color:var(--error);border-color:rgba(239,68,68,.2)" onclick="delCoupon('${c.id}')" title="${t('deleteProduct')}"><i class="fas fa-trash"></i></button>
        </div></div>`}).join('')}${!coupons.length?`<div class="empty-state"><i class="fas fa-ticket"></i><h3>${t('noCoupons')}</h3></div>`:''}</div></div>`)}

function showCouponForm(c=null){
  const isE=!!c;
  const m=document.createElement('div');m.className='modal-overlay';m.onclick=e=>{if(e.target===m)m.remove()};
  m.innerHTML=`<div class="modal-content" style="max-width:550px;max-height:85vh;overflow-y:auto">
    <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:1.2rem;padding-bottom:.8rem;border-bottom:2px solid var(--accent)">
      <i class="fas fa-ticket" style="color:var(--accent);font-size:1.2rem"></i>
      <h2 style="font-size:1.1rem;margin:0">${isE?t('editCoupon'):t('addCoupon')}</h2>
      <button onclick="this.closest('.modal-overlay').remove()" style="margin-inline-start:auto;background:none;border:none;cursor:pointer;font-size:1.2rem;color:var(--text-secondary);padding:4px"><i class="fas fa-times"></i></button>
    </div>
    <form onsubmit="saveCoupon(event,${isE?`'${c.id}'`:'null'})">
      <div class="form-grid">
        <div class="form-group"><label>${t('couponCode')} *</label><input type="text" id="cpCode" class="form-input" value="${c?.code||''}" required style="text-transform:uppercase;font-family:monospace;letter-spacing:2px"></div>
        <div class="form-group"><label>${t('couponType')}</label><select id="cpType" class="form-input"><option value="percentage" ${c?.discount_type==='percentage'||!c?'selected':''}>${t('percentage')} %</option><option value="fixed" ${c?.discount_type==='fixed'?'selected':''}>${t('fixedAmount')} ${t('currency')}</option></select></div>
        <div class="form-group"><label>${t('couponDiscount')} *</label><input type="number" id="cpValue" class="form-input" value="${c?.discount_value||''}" min="1" required></div>
        <div class="form-group"><label>${t('minOrder')} (${t('currency')})</label><input type="number" id="cpMin" class="form-input" value="${c?.min_order||0}" min="0"></div>
        <div class="form-group"><label>${t('maxUses')} (0 = ${t('unlimited')})</label><input type="number" id="cpMax" class="form-input" value="${c?.max_uses||0}" min="0"></div>
        <div class="form-group"><label>${t('expiresAt')}</label><input type="date" id="cpExpiry" class="form-input" value="${c?.expires_at?new Date(c.expires_at).toISOString().split('T')[0]:''}"></div>
        <div class="form-group"><label>${t('active')}</label><select id="cpActive" class="form-input"><option value="1" ${c?.is_active!==false?'selected':''}>${t('active')}</option><option value="0" ${c?.is_active===false?'selected':''}>${t('inactive')}</option></select></div>
      </div>
      <div id="cpErr" class="form-error" style="margin:.6rem 0"></div>
      <div style="display:flex;gap:.5rem;margin-top:1rem"><button type="submit" class="btn-primary" id="cpBtn"><i class="fas fa-save"></i> ${t('save')}</button><button type="button" class="btn-secondary" onclick="this.closest('.modal-overlay').remove()"><i class="fas fa-times"></i> ${t('cancel')}</button></div>
    </form></div>`;document.body.appendChild(m)}

async function saveCoupon(e,id){
  e.preventDefault();
  const data={code:document.getElementById('cpCode').value,discount_type:document.getElementById('cpType').value,discount_value:document.getElementById('cpValue').value,min_order:document.getElementById('cpMin').value,max_uses:document.getElementById('cpMax').value,expires_at:document.getElementById('cpExpiry').value||null,is_active:document.getElementById('cpActive').value==='1'};
  const btn=document.getElementById('cpBtn');btn.innerHTML='<div class="loading-spinner" style="width:18px;height:18px"></div>';
  const{ok,data:res}=id?await API.put(`/api/coupons/${id}`,data):await API.post('/api/coupons',data);
  if(ok){document.querySelector('.modal-overlay')?.remove();showToast(id?t('couponUpdated'):t('couponAdded'));loadAdminCouponsPage()}
  else{document.getElementById('cpErr').textContent=res.error||t('error');btn.innerHTML=`<i class="fas fa-save"></i> ${t('save')}`}}

async function delCoupon(id){if(!confirm(t('confirmDelete')))return;const{ok}=await API.del(`/api/coupons/${id}`);if(ok){showToast(t('couponDeleted'));loadAdminCouponsPage()}}
...
async function initApp(){
  ...
  Router.register('/admin/coupons',loadAdminCouponsPage);
